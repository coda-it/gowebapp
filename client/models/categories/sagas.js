...
function callAddCategory(name, image) {
...
export function* onAddCategory({ name, image, }) {
...
function callUpdateCategory(name, image, id) {
    const request = new Request(constants.CATEGORY_ENDPOINT, {
        method: 'PUT',
        body: JSON.stringify({
            id,
...
export function* onUpdateCategory({ id, name, image, }) {
    ...
    yield put(alertActions.addAlert('Category updated', alertConstants.ALERT_TYPE_INFO));
}
